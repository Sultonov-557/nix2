...
```bash
git clone https://github.com/your-username/nixul.git /mnt/etc/nixos
cd /mnt/etc/nixos
```
...
2. **Choose a Flavor:** Open up `flake.nix` in the root of the repository. You'll see `nixosConfigurations`. Pick your host and set the `flavor` attribute to the flavor you want.

    ```nix
    # in flake.nix
    nixosConfigurations = {
      "home-pc" = lib.mkSystem {
        hostname = "home-pc";
        hostsDir = ./nix/hosts;
        flavors = flavors;
        flavor = "catppuccin"; # <-- Change this to your desired flavor!
      };
      # ... other hosts
    };
    ```
